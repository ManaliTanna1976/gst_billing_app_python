...
def invoice_data_validator(invoice_data):
    ...
    if len(invoice_data['customer-pan']) != 10 and len(invoice_data['customer-pan']) != 0:
        print("Error: Incorrect Customer PAN")
        return "Error: Incorrect Customer PAN"
    return None
...
def invoice_data_processor(invoice_post_data):
    ...
    processed_invoice_data['invoice_payment_type'] = invoice_post_data['invoice-payment-type']
    ...
    processed_invoice_data['customer_pan'] = invoice_post_data['customer-pan']
    ...
    for idx, product in enumerate(invoice_post_data['invoice-product']):
        if product:
            ...
            item_entry['invoice_rate_without_gst'] = float(invoice_post_data['invoice-rate-with-out-gst-new'][idx])
    ...
    for item in invoice_data_processed['items']:
        ...
        if Product.objects.filter(user=request.user,
            ...
            product = Product(user=request.user,
                              ...
                              product_hsn=item['invoice_hsn'],
                              product_gst_percentage=item['invoice_gst_percentage'])
        product.product_rate_without_gst = item['invoice_rate_without_gst']
        product.save()
