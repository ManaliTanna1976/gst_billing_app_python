...
function update_invoice_totals() {
    ...
    // amount gst
    sum_amt_gst = 0
    $('input[name=invoice-amt-gst]').each(function(){
        sum_amt_gst += parseFloat($(this).val());
    });
    $('input[name=invoice-total-amt-gst]').val(sum_amt_gst.toFixed(2));
...
function initialize_auto_calculation(){
    ...
    $('input[name=invoice-qty], input[name=invoice-gst-percentage], input[name=invoice-rate-with-out-gst-new]').change(function (){
        console.log($(this));
        update_amounts($(this));
    });
}
...
function update_amounts(element){
    ...
    var rate_without_gst_new = parseFloat(element.parent().parent().find('input[name=invoice-rate-with-out-gst-new]').val());
    var gst_percentage = parseFloat(element.parent().parent().find('input[name=invoice-gst-percentage]').val());
    console.log(rate_without_gst_new);
    console.log(qty);
    var amt_without_gst = rate_without_gst_new * qty;
    ...
    var amt_gst = cgst + sgst + igst;
    element.parent().parent().find('input[name=invoice-amt-gst]').val(amt_gst.toFixed(2));
