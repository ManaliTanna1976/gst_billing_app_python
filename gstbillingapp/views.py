    ...
    if request.method == 'POST':
        ...
        try:
            customer = Customer.objects.get(user=request.user,
                                            ...
                                            customer_pan=invoice_data['customer-pan'])
        except:
            ...
            print(invoice_data['customer-pan'])
...
def invoice_viewer(request, invoice_id):
    ...
    if context['invoice_data'].get('invoice_payment_type'):
        context['invoice_data']['invoice_payment_type'] = context['invoice_data']['invoice_payment_type'].replace("_", " ").title()
    context['currency'] = "₹"
    context['total_in_words'] = num2words.num2words(int(context['invoice_data']['invoice_total_amt_with_gst']), lang='en_IN').title()
    user_profile.get_business_bank_details_list = user_profile.business_bank_details.split(',')
    context['user_profile'] = user_profile
